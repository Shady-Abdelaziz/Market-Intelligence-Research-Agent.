...
def test_sector_correlation_fires_above_threshold():
    fired, _ = trigger_sector_correlation(
        _state_with({"correlation": {"vs_sector_etf": 0.97}})
    )
...
def test_sector_correlation_does_not_fire_at_threshold():
    fired, _ = trigger_sector_correlation(
        _state_with({"correlation": {"vs_sector_etf": 0.95}})
    )
...
def test_neutral_sentiment_perfectly_neutral_fires():
    fired, _ = trigger_neutral_sentiment(
        _state_with(
            {"news_sentiment": {"distribution": {"positive": 0, "negative": 0, "neutral": 5, "total": 5}}}
        )
    )
    assert fired is True
...
def test_neutral_sentiment_evenly_split_fires():
    fired, _ = trigger_neutral_sentiment(
        _state_with(
            {"news_sentiment": {"distribution": {"positive": 1, "negative": 1, "neutral": 3, "total": 5}}}
        )
    )
    assert fired is True
...
def test_neutral_sentiment_clear_skew_no_fire():
    fired, _ = trigger_neutral_sentiment(
        _state_with(
            {"news_sentiment": {"distribution": {"positive": 4, "negative": 0, "neutral": 1, "total": 5}}}
        )
    )
    assert fired is False
